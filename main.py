...
def get_crewai_agent(ticker: str, year: str):
    ...
    """
    sec_tools_list, earnings_calls_tools_list, books_tool = build_database_and_query(
        ticker=ticker, year=year
    )
    sec_filings_agent, earnings_call_transcripts_agent, books_agents = get_agents(
        sec_tools_list=sec_tools_list,
        earnings_calls_tools_list=earnings_calls_tools_list,
        books_tool=books_tool,
    )
    sec_task, earnings_call_task, books_task = get_tasks(
        sec_tools_list,
        sec_filings_agent,
