from data_source.vectorDatabaseDocker import create_database
...
def build_database_and_query(ticker: str, year: str):
    ...
    query_sec = partial(
        query_database_sec, qdrant_client=qdrant_client, encoder=encoder
    )
    query_earnings_call = partial(
        query_database_earnings_call, qdrant_client=qdrant_client, encoder=encoder
    )
    sec_tools_list = []
    ...
    for idx, speaker_list in enumerate(
        [speakers_list_1, speakers_list_2, speakers_list_3, speakers_list_4]
    ):
        if speaker_list == []:
            continue
        else:
            ...
            def earnings_call_tool(question: str) -> str:
                ...
                """
                return query_earnings_call(
                    question=question, quarter=quarter, speakers_list=speaker_list
                )
